...
| ベースモデル | `qwen3:8b` |
| 量子化 | Q4_K_M |
| テンプレート形式 | ChatML形式（`<\|im_start\|>`/`<\|im_end\|>`） |
| カスタムモデル名 | `dev-prompt-refiner-qwen3-8b` |

## パラメータ設定

| パラメータ | 値 | 根拠 |
|---|---|---|
| `temperature` | 0.7 | Qwen公式推奨。非Thinkingモードでの最適値。0.6未満ではThinkingモード時に無限ループ発生 |
| `top_k` | 20 | Qwen公式推奨値 |
| `top_p` | 0.8 | Qwen公式推奨（非Thinkingモード用） |
| `repeat_penalty` | 1.0（無効） | `presence_penalty` で繰り返し制御するため無効化 |
| `presence_penalty` | 1.5 | Q4_K_M量子化版の繰り返しループ対策。Qwen公式推奨。1.5超で日本語に他言語混在の可能性あり |
| `stop` | `<\|im_end\|>` | ChatML形式の終了トークン |

## Thinkingモードについて

**プロンプト整形タスクではThinkingモードの無効化を推奨します。**

| 観点 | Thinkingモード ON | Thinkingモード OFF |
|---|---|---|
| 用途 | 複雑な推論・数学・コーディング | 指示追従・テキスト変換 |
| レイテンシ | 高い（思考プロセス分の時間） | 低い |
| 出力 | `<think>...</think>` + 回答 | 回答のみ |
| temperature下限 | 0.6（これ未満でループ） | 制限なし（公式推奨0.7） |

本Modelfileではシステムプロンプトの先頭・末尾に `/no_think` を配置してThinkingモードを無効化しています。

### `/no_think` 以外の無効化方法

1. **APIレベル**: リクエストに `"think": false` を追加（最も確実）
2. **Ollama CLI**: `ollama run qwen3:8b --think=false "..."`
3. **Modelfile PARAMETER**（PR進行中、将来的に `PARAMETER think false` が使用可能になる見込み）

## 注意事項

- **`presence_penalty 1.5` は必須**: Q4_K_M量子化版は繰り返しループが発生しやすい既知の問題があります。1.5を維持してください
- **temperatureを下げすぎない**: 他モデルのように0.1にすると品質が低下します。Qwen3は0.7が公式推奨値です
- **`seed` は非設定**: temperature 0.7では固定シードの再現性効果が薄いため省略
...
notepad "$env:TEMP\Modelfile-qwen3-8b"

# 2. カスタムモデルを作成
ollama create dev-prompt-refiner-qwen3-8b -f "$env:TEMP\Modelfile-qwen3-8b"

# 3. 動作確認
ollama run dev-prompt-refiner-qwen3-8b "えーとリアクトでログインフォーム作りたいんだけどメールアドレスとパスワードのバリデーション付きでお願いします"
```
