...
| 項目 | 内容 |
|---|---|
| ベースモデル | `gemma3n:e4b-it-q8_0` |
| 量子化 | Q8_0 |
| テンプレート形式 | Gemma形式（`<start_of_turn>`/`<end_of_turn>`） |
| カスタムモデル名 | `dev-prompt-refiner-gemma3n` |

## パラメータ設定

| パラメータ | 値 | 根拠 |
|---|---|---|
| `temperature` | 0.1 | Google公式: 0.2-0.5が「集中的・決定論的」。整形タスクでは更に低い0.1が適切 |
| `top_k` | 10 | デフォルト64は創造的生成向け。構造化出力では10で十分 |
| `top_p` | 0.7 | 低temperatureとの組み合わせでサンプリングプールを制限 |
| `repeat_penalty` | 1.0（無効） | Google公式推奨。Markdownの `#` `-` の繰り返しへのペナルティを防止 |
| `seed` | 42 | 固定シードで出力の再現性を向上 |
| `stop` | `<end_of_turn>` | Gemma系の終了トークン |

## 注意事項

- **systemロールなし**: Gemma系モデルにはsystemロールが存在しません。システムプロンプトはuserターン内に埋め込みます
- **Q8版の明示的指定が必要**: OllamaのデフォルトタグはQ4_K_M。Q8版を使うには `gemma3n:e4b-it-q8_0` を明示的に指定すること
- **実効パラメータ数**: 総パラメータ8Bのうち、実効パラメータは約4B（MatFormer技術による選択的パラメータ活性化）。Q8量子化により精度損失が少なく、構造化出力の安定性が高い
- **最大コンテキスト長**: 32K（gemma-3-12b-itの128Kより短い）。プロンプト整形には十分
...
notepad "$env:TEMP\Modelfile-gemma3n"

# 2. カスタムモデルを作成
ollama create dev-prompt-refiner-gemma3n -f "$env:TEMP\Modelfile-gemma3n"

# 3. 動作確認
ollama run dev-prompt-refiner-gemma3n "えーとリアクトでログインフォーム作りたいんだけどメールアドレスとパスワードのバリデーション付きでお願いします"
```
