...
| カスタムモデル名 | `dev-prompt-refiner-gemma12b` |
...
- **KVキャッシュ量子化の禁止**: `OLLAMA_KV_CACHE_TYPE` 環境変数は設定しないこと（速度が激減する既知バグ）
...
# 3. 動作確認
ollama run dev-prompt-refiner-gemma12b "えーとリアクトでログインフォーム作りたいんだけどメールアドレスとパスワードのバリデーション付きでお願いします"
```
